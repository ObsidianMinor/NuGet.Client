    ...
    public class Program
    {
        ...
        public static int MainCore(string workingDirectory, string[] args)
        {
            ...
            catch (AggregateException exception)
            {
                ...
                if (unwrappedEx is PathTooLongException)
                {
        ...
        private static void LogHelperMessageForPathTooLongException(Console logger)
        {
            ...
            {
                logger.WriteWarning(LocalizedResourceManager.GetString(nameof(NuGetResources.Warning_LongPath_UnsupportOS)));
            }
        ...
        {
            var reg = RegistryKeyUtility.OpenSubKey(OSVersionRegistryKey, Registry.LocalMachine, logger);

            var productName = (string)RegistryKeyUtility.GetValue("ProductName", reg, logger);

            RegistryKeyUtility.Close(reg);
        ...
        {
            var reg = RegistryKeyUtility.OpenSubKey(FilesystemRegistryKey, Registry.LocalMachine, logger);

            var longPathsEnabled = RegistryKeyUtility.GetValue("LongPathsEnabled", reg, logger);
        ...
        {
            var reg = RegistryKeyUtility.OpenSubKey(DotNetSetupRegistryKey, Registry.LocalMachine, logger);

            var release = RegistryKeyUtility.GetValue("Release", reg, logger);

            RegistryKeyUtility.Close(reg);

            return release != null && (int)release >= 394802;
        }
    }
}
